...
public class InterfaceGui implements Interface {
	...
	private PiecesOnTheMap createSheepPanel(TypeOfSheep type) {
		...
		switch (type) {
			case MALESHEEP :
				...
				break;
			case FEMALESHEEP :
				toReturn = new SheepPanel(
						ImagePathCreator.findSheepPathNoNumber(),
						dimCalc.getSheepDimension());
				break;
			case NORMALSHEEP :
				toReturn = new LambPanel(
						ImagePathCreator.findLambPathNoNumber(),
	...
	public synchronized List<MarketOffer> askMarketOffers() {
		...
		List<MarketOffer> toReturn = offersOfAPlayer;
	...
	public synchronized List<MarketBuy> askMarketBuy(List<MarketOffer> offers) {
		...
		MarketPanelBuy marketPanel = new MarketPanelBuy(offers, this);
		frame.validate();
		frame.repaint();
		...
		List<MarketBuy> toReturn = purchasedOfferOfPlayer;
